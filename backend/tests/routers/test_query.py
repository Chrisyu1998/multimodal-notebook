...
hybrid_search and generation are mocked so these tests run without any
real Gemini calls or ChromaDB I/O.

Covers:
  - Happy path: question → hybrid_search → chunks → answer
  - Empty result from hybrid_search → canned "no information" response
...
def _mock_collection_not_empty():
...
class TestQueryHappyPath:
    ...
    def test_response_contains_answer(self):
        with patch("backend.routers.query.hybrid_search", return_value=_FAKE_CHUNKS), \
             patch("backend.routers.query.generation.generate_answer", return_value=_FAKE_GENERATION_RESULT):
            resp = client.post("/query/", json={"question": "What is the capital of France?"})
        assert resp.json()["answer"] == "Paris is the capital of France."
    ...
    def test_hybrid_search_called_with_question(self):
        ...
        with patch("backend.routers.query.hybrid_search", mock_search), \
             patch("backend.routers.query.generation.generate_answer", return_value=_FAKE_GENERATION_RESULT):
            client.post("/query/", json={"question": "What is the capital of France?"})
        mock_search.assert_called_once_with("What is the capital of France?", top_k=config.VECTOR_TOP_K)
...
# ---------------------------------------------------------------------------
# Edge case: no chunks returned from hybrid_search
# ---------------------------------------------------------------------------

class TestQueryNoChunks:
    def test_returns_200_when_no_chunks(self):
        with patch("backend.routers.query.hybrid_search", return_value=[]):
...
class TestQueryServiceFailures:
    def test_503_when_hybrid_search_raises_embedding_error(self):
        err = EmbeddingBatchError("query", [-1], RuntimeError("Gemini down"))
        with patch("backend.routers.query.hybrid_search", side_effect=err):
            resp = client.post("/query/", json={"question": "test"})
        assert resp.status_code == 503
        assert "embedding" in resp.json()["detail"].lower()

    def test_503_when_hybrid_search_raises_vectorstore_error(self):
        with patch("backend.routers.query.hybrid_search",
                   side_effect=VectorStoreUnavailableError("ChromaDB down")):
            resp = client.post("/query/", json={"question": "test"})
        assert resp.status_code == 503

    def test_502_when_hybrid_search_raises_generic_error(self):
        with patch("backend.routers.query.hybrid_search",
    ...
    def test_502_when_generation_raises_generic_error(self):
        with patch("backend.routers.query.hybrid_search", return_value=_FAKE_CHUNKS), \
             patch("backend.routers.query.generation.generate_answer",
                   side_effect=RuntimeError("unexpected")):
            resp = client.post("/query/", json={"question": "test"})
    ...
    def test_generation_not_called_when_search_fails(self):
        mock_gen = MagicMock()
        with patch("backend.routers.query.hybrid_search", side_effect=RuntimeError("down")), \
             patch("backend.routers.query.generation.generate_answer", mock_gen):
...
class TestQueryInputValidation:
    ...
    def test_whitespace_question_is_passed_as_is(self):
        ...
        mock_search.assert_called_once_with("   ", top_k=config.VECTOR_TOP_K)
    ...
    def test_question_with_special_characters(self):
        ...
        mock_search.assert_called_once_with("What is 2+2? ∑ ñoño 日本語", top_k=config.VECTOR_TOP_K)
