...
_AUDIO_CONTENT = b"fake mp3 content"
_AUDIO_HASH = hashlib.sha256(_AUDIO_CONTENT).hexdigest()

# Fake audio chunks mirroring the chunk_audio output shape
_FAKE_AUDIO_CHUNKS = [
    {
        "type": "audio",
        "audio_bytes": b"fake-mp3-clip-0",
        "text": "Hello, welcome to the meeting.",
        "source": "/tmp/recording.mp3",
        "page": 0,
        "chunk_index": 0,
        "modality": "audio_clip",
        ...
        "transcript_text": "Hello, welcome to the meeting.",
        "forced_split": False,
    },
    {
        "type": "audio",
        "audio_bytes": b"fake-mp3-clip-1",
        ...
        "transcript_text": "Thanks for having me.",
        "forced_split": False,
    },
]
_FAKE_AUDIO_EMBEDDED = [{"embedding": [0.7, 0.8], **c} for c in _FAKE_AUDIO_CHUNKS]

# Single-chunk variant for simpler tests
_FAKE_AUDIO_CHUNKS_SINGLE = [_FAKE_AUDIO_CHUNKS[0]]
_FAKE_AUDIO_EMBEDDED_SINGLE = [_FAKE_AUDIO_EMBEDDED[0]]
...
class TestChunkerDispatch:
    ...
    @pytest.mark.parametrize("filename", ["talk.mp3", "interview.wav", "podcast.m4a"])
    def test_audio_extensions_call_chunk_audio(self, tmp_path, filename):
        mock_audio = MagicMock(return_value=_FAKE_AUDIO_CHUNKS)
        mock_pdf   = MagicMock(return_value=_FAKE_CHUNKS)

        with patch("backend.config.TMP_UPLOADS_DIR", str(tmp_path)), \
             patch("backend.routers.upload.vectorstore.is_file_indexed", return_value=False), \
             patch("backend.routers.upload.chunking.chunk_audio", mock_audio), \
             patch("backend.routers.upload.chunking.chunk_pdf", mock_pdf), \
             patch("backend.routers.upload.embeddings.embed_chunks", return_value=_FAKE_AUDIO_EMBEDDED), \
             patch("backend.routers.upload.vectorstore.add_chunks"):
            _upload(filename, _AUDIO_CONTENT)
...
class TestAudioIngestion:
    ...
    def test_returns_200(self, tmp_path, filename):
        with patch("backend.config.TMP_UPLOADS_DIR", str(tmp_path)), \
             ...
             patch("backend.routers.upload.embeddings.embed_chunks", return_value=_FAKE_AUDIO_EMBEDDED), \
             patch("backend.routers.upload.vectorstore.add_chunks"):
            resp = _upload(filename, _AUDIO_CONTENT)
        assert resp.status_code == 200
    ...
    def test_status_is_indexed(self, tmp_path, filename):
        with patch("backend.config.TMP_UPLOADS_DIR", str(tmp_path)), \
             ...
             patch("backend.routers.upload.embeddings.embed_chunks", return_value=_FAKE_AUDIO_EMBEDDED), \
             patch("backend.routers.upload.vectorstore.add_chunks"):
            body = _upload(filename, _AUDIO_CONTENT).json()
        assert body["status"] == "indexed"
    ...
    def test_pipeline_call_order(self, tmp_path):
        """chunk_audio → embed_chunks → add_chunks must be called in that order."""
        call_order = []
        mock_chunk = MagicMock(side_effect=lambda *a, **kw: call_order.append("chunk") or _FAKE_AUDIO_CHUNKS)
        mock_embed = MagicMock(side_effect=lambda *a, **kw: call_order.append("embed") or _FAKE_AUDIO_EMBEDDED)
        mock_add   = MagicMock(side_effect=lambda *a, **kw: call_order.append("add"))

        with patch("backend.config.TMP_UPLOADS_DIR", str(tmp_path)), \
             patch("backend.routers.upload.vectorstore.is_file_indexed", return_value=False), \
             patch("backend.routers.upload.chunking.chunk_audio", mock_chunk), \
             patch("backend.routers.upload.embeddings.embed_chunks", mock_embed), \
             patch("backend.routers.upload.vectorstore.add_chunks", mock_add):
            _upload("talk.mp3", _AUDIO_CONTENT)

        assert call_order == ["chunk", "embed", "add"]

    def test_chunk_audio_called_with_temp_file_path(self, tmp_path):
        """chunk_audio receives the saved temp path, not the original filename."""
        mock_chunk = MagicMock(return_value=_FAKE_AUDIO_CHUNKS)
        ...
        called_path = mock_chunk.call_args[0][0]
        assert called_path.startswith(str(tmp_path))
        assert "talk.mp3" in called_path

    def test_all_chunks_stamped_with_original_filename(self, tmp_path):
        """source on every audio chunk must be the original filename."""
        captured = {}
        def capture_embed(chunks):
            captured["chunks"] = chunks
            return _FAKE_AUDIO_EMBEDDED
    ...
    def test_all_chunks_stamped_with_file_hash(self, tmp_path):
        ...
        expected_hash = hashlib.sha256(_AUDIO_CONTENT).hexdigest()
    ...
    def test_add_receives_embedded_output(self, tmp_path):
        ...
        mock_add.assert_called_once_with(_FAKE_AUDIO_EMBEDDED)

    def test_chunk_pdf_not_called_for_audio(self, tmp_path):
        """chunk_pdf must never be invoked for an audio upload."""
        mock_pdf = MagicMock()
    ...
    def test_chunk_video_not_called_for_audio(self, tmp_path):
        """chunk_video must never be invoked for an audio upload."""
...
class TestAudioCornerCases:
    ...
    def test_audio_dedup_skips_chunk_audio(self):
        """chunk_audio must not be called when the content is already indexed."""
        mock_chunk = MagicMock()
        with patch("backend.routers.upload.vectorstore.is_file_indexed", return_value=True), \
             patch("backend.routers.upload.chunking.chunk_audio", mock_chunk):
            _upload("talk.mp3", _AUDIO_CONTENT)
    ...
    def test_add_chunks_not_called_after_audio_embed_failure(self, tmp_path):
        """If embed_chunks raises, add_chunks must not be called."""
        mock_add = MagicMock()
        with patch("backend.config.TMP_UPLOADS_DIR", str(tmp_path)), \
             patch("backend.routers.upload.vectorstore.is_file_indexed", return_value=False), \
    ...
    def test_multi_speaker_chunks_both_stamped(self, tmp_path):
        """When chunk_audio returns multiple speaker chunks, every one gets source + file_hash."""
        captured = {}
        def capture_embed(chunks):
            captured["chunks"] = chunks
            return _FAKE_AUDIO_EMBEDDED
    ...
    def test_single_chunk_stamped_when_one_speaker(self, tmp_path):
        """Single-chunk audio (one speaker, short file) still gets stamped correctly."""
        captured = {}
        def capture_embed(chunks):
            captured["chunks"] = chunks
            return _FAKE_AUDIO_EMBEDDED_SINGLE

        with patch("backend.config.TMP_UPLOADS_DIR", str(tmp_path)), \
             patch("backend.routers.upload.vectorstore.is_file_indexed", return_value=False), \
             patch("backend.routers.upload.chunking.chunk_audio",
                   return_value=_FAKE_AUDIO_CHUNKS_SINGLE), \
             patch("backend.routers.upload.embeddings.embed_chunks", side_effect=capture_embed), \
             patch("backend.routers.upload.vectorstore.add_chunks"):
            body = _upload("talk.mp3", _AUDIO_CONTENT).json()
        ...
        assert body["status"] == "indexed"
        assert len(captured["chunks"]) == 1
        assert captured["chunks"][0]["source"] == "talk.mp3"
        assert captured["chunks"][0]["modality"] == "audio_clip"

    def test_empty_chunk_list_from_chunk_audio_still_calls_embed(self, tmp_path):
        """If chunk_audio returns [], embed_chunks and add_chunks are still called."""
        mock_embed = MagicMock(return_value=[])
        mock_add = MagicMock()

        with patch("backend.config.TMP_UPLOADS_DIR", str(tmp_path)), \
             patch("backend.routers.upload.vectorstore.is_file_indexed", return_value=False), \
             patch("backend.routers.upload.chunking.chunk_audio", return_value=[]), \
             patch("backend.routers.upload.embeddings.embed_chunks", mock_embed), \
             patch("backend.routers.upload.vectorstore.add_chunks", mock_add):
            body = _upload("talk.mp3", _AUDIO_CONTENT).json()
    ...
    def test_dedup_check_uses_content_hash_not_filename_for_audio(self):
        """Same audio filename with different bytes → not a duplicate."""
        mock_add = MagicMock()
        with patch("backend.routers.upload.vectorstore.is_file_indexed", return_value=False), \
             patch("backend.routers.upload.chunking.chunk_audio", return_value=_FAKE_AUDIO_CHUNKS), \
             patch("backend.routers.upload.embeddings.embed_chunks", return_value=_FAKE_AUDIO_EMBEDDED), \
             patch("backend.routers.upload.vectorstore.add_chunks", mock_add):
            _upload("talk.mp3", b"audio version 1")
            _upload("talk.mp3", b"audio version 2")
        assert mock_add.call_count == 2

    def test_is_file_indexed_called_with_sha256_of_audio_content(self):
        """is_file_indexed must receive the SHA-256 of the audio bytes."""
        expected_hash = hashlib.sha256(_AUDIO_CONTENT).hexdigest()
        mock_is_indexed = MagicMock(return_value=True)
