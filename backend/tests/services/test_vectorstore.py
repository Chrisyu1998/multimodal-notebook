...
# ---------------------------------------------------------------------------
# search_balanced tests
# ---------------------------------------------------------------------------

def test_search_balanced_injects_missing_modality(store):
    """A modality absent from the global top-k is injected via a filtered search."""
    doc_chunk = make_chunk("Text.", "doc.pdf", 1, 0, unit_vector(), type_="document")
    # orthogonal_vector scores ~0 against unit_vector query — won't reach global top-1
    video_chunk = make_chunk(
        "Video.", "video.mp4", 0, 1, orthogonal_vector(),
        file_hash="b" * 64, modality="video_clip", type_="video",
    )
    vs.add_chunks([doc_chunk, video_chunk])

    results = vs.search_balanced(unit_vector(), global_top_k=1, top_k_per_modality=1)
    types_returned = {r["type"] for r in results}
    assert "document" in types_returned
    assert "video" in types_returned


def test_search_balanced_no_injection_when_type_not_in_collection(store):
    """No error when collection has no chunks of a missing type."""
    doc_chunk = make_chunk("Text.", "doc.pdf", 1, 0, unit_vector(), type_="document")
    vs.add_chunks([doc_chunk])

    # Collection only has documents; video/audio/image are absent — should not raise
    results = vs.search_balanced(unit_vector(), global_top_k=20, top_k_per_modality=5)
    assert len(results) == 1
    assert results[0]["type"] == "document"


def test_search_balanced_deduplicates_injected_chunks(store):
    """Chunks already in global results are not re-added during injection."""
    doc_chunk = make_chunk("Text.", "doc.pdf", 1, 0, unit_vector(), type_="document")
    video_chunk = make_chunk(
        "Video.", "video.mp4", 0, 1, orthogonal_vector(),
        file_hash="b" * 64, modality="video_clip", type_="video",
    ...
    results = vs.search_balanced(unit_vector(), global_top_k=1, top_k_per_modality=5)
    ids = [(r["source"], r["chunk_index"]) for r in results]
    assert len(ids) == len(set(ids))


def test_search_balanced_results_sorted_by_score_descending(store):
    """Merged results are sorted highest score first."""
    doc_chunk = make_chunk("High.", "doc.pdf", 1, 0, unit_vector(), type_="document")
    video_chunk = make_chunk(
        "Low.", "video.mp4", 0, 1, orthogonal_vector(),
        file_hash="b" * 64, modality="video_clip", type_="video",
    )
    vs.add_chunks([doc_chunk, video_chunk])
